...
- 分派 Owner、整改人和验证人时支持按姓名或用户名快速搜索。
...
登录页必须输入用户名和密码，前端不再内置账号密码或提供免密角色切换。需要体验其他角色时，请先退出当前账号，再使用对应账号重新登录。
...
4. 退出管理员账号，使用整改人员账号重新登录，进入“我的待办”并提交整改说明和证据链接。
5. 退出后使用验证人员账号重新登录，对整改结果进行验证；通过后风险关闭，驳回后返回整改中。
